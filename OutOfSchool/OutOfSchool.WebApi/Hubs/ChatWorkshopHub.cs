...
namespace OutOfSchool.WebApi.Hubs
{
    ...
    public class ChatWorkshopHub : Hub
    {
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            logger.Debug($"New Hub-connection established. {nameof(userId)}: {userId}, {nameof(userRole)}: {userRole}");
        ...
        {
            var userId = Context.User.FindFirst("sub")?.Value;
            logger.Debug($"UserId: {userId} connection:{Context.ConnectionId} disconnected.");
        ...
        {
            logger.Debug($"{nameof(SendMessageToOthersInGroupAsync)}.Invoked.");
