...
public class AdminControllerTests
{
    ...
    public async Task LicenseStatusUpdate_WhenInvalidRequest_ReturnsBadRequestObjectResult()
    {
        ...
        sensitiveProviderService.Setup(x => x.UpdateLicenseStatus(updateRequest, userId))
            .ThrowsAsync(new ArgumentException(errorMessage));

        // Act
        var result = await controller.LicenseStatusProviderUpdate(updateRequest).ConfigureAwait(false);
    ...
    public async Task StatusUpdate_WhenIdDoesNotExist_ReturnsNotFoundResult()
    {
        ...
        sensitiveProviderService.Setup(x => x.UpdateStatus(updateRequest, userId))
            .ReturnsAsync(null as ProviderStatusDto);

        // Act
        var result = await controller.StatusProviderUpdate(updateRequest).ConfigureAwait(false);
