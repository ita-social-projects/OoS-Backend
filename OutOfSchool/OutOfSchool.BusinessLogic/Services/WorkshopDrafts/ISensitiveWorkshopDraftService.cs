...
public interface ISensitiveWorkshopDraftService
{
    ...
    /// The task result contains the <see cref="SearchResult{TDto}"/> that contains found elements.</returns>    
    Task<SearchResult<TDto>> FetchByFilterForAdmins<TDto>(WorkshopDraftFilterAdministration filter = null) where TDto : class;
}